<script type="text/javascript">
    ...
    $(function() {
        ...
        $('.row-container-step3').on('click', '.add-icon-step3', function(e) {
            e.preventDefault();
            // Create the HTML for the new row
            var newRow = $(' <div class="row data_row">\
                        <div class="form-group mb-3 col-md-2 col-12">\
                            <label for="exampleInputHeight">Height</label>\
                            <input type="number"  name="height[]" class="form-control height" id="exampleInputheights" placeholder="Enter Height">\
                        </div>\
                        <div class="form-group mb-3 col-md-2 col-12">\
                            <label for="exampleInputWeight">Weight</label>\
                            <input type="number" name="weight[]" class="form-control weight" id="exampleInputWeight" placeholder="Enter Weight">\
                        </div>\
                        <div class="form-group mb-3 col-md-1 col-12">\
                            <label for="exampleInputWidth">Width</label>\
                            <input type="number" name="width[]" class="form-control width" id="exampleInputWidth" placeholder="Enter Width">\
                        </div>\
                        <div class="form-group mb-3 col-md-2 col-12">\
                            <label for="exampleInputGunfoot">Gunfoot</label>\
                            <input type="number" name="gunfoot[]" readonly class="form-control gunfoot" id="exampleInputGunfoot" placeholder="Enter Gunfoot">\
                        </div>\
                        <div class="form-group mb-3 col-md-2 col-12">\
                        ...
                        <div class="mb-3 col-md-1 col-12">\
                            <div style="display:flex">\
                                <a class="btn btn-info btn-sm mt-4 add-icon-step3  ml-2 mr-1">\
        ...
        $.ajax({
            ...
            success: function(result) {
                if (result.status) {
                    ...
                    setStep(3, result.id);
                    $(".wipId").val(result.id);
                    $(".finish_good").val(result.no_of_pieces)
                }
                $('.submitBtn').prop('disabled', false);
            },
            error: function(data) {
            ...
            $.ajax({
                ...
                success: function(result) {
                    if (result.status) {
                        ...
                        url = url.replace("wip_id", result.id); 
                        $(".wipId").val(result.id);
                        window.location = url;
    ...
    $(document).on('keyup', '.height,.weight,.width', function(event) {
        var height = parseFloat($(this).closest('.data_row').find('.height').val());
        var weight = parseFloat($(this).closest('.data_row').find('.weight').val());
        var width = parseFloat($(this).closest('.data_row').find('.width').val());
        console.log("height",height," ",weight," ",width);
        $(this).closest('.data_row').find('.gunfoot').val(height*weight*width);
    });
