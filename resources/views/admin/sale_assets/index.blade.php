...
<script type="text/javascript">
 var oTable;
         ...
         oTable = $('#example1').DataTable({
            ...
            columns: [{
                ...
                },
               {
                    ...
                    render: (row, index) => {
                        console.log("row",row);
                        return row?.assets?.machine_name;
                    }
                },
                {
                    data: 'quantity',
    ...
    function editassignasset(id) {
        ...
        $.ajax({
            ...
            success: function(data) {
                getAssets(data.assign_assets.assets_category,data.assign_assets.assets_name)
                // $("#editdata").html(data);
    ...
    if(key == "assets_category" || key == "assets_type" || key == "transfer_mine_name")
    ...
    <script>
        ...
        $(".quantity").on("keyup", function() {
            var val = $(this).val();
          var assign_qty =  $(".assign_qty").val();
            console.log(val, " ", assign_qty);
            if (parseFloat(val) > parseFloat(assign_qty)) {
                console.log( $(this).closest("form").find($(".qty_err")));
                // $("#quantity").closest(".form-group").append("<span class='text-danger'>Quantity doen't allow greater than "+qty+"</span>")
                $(this).closest("form").find($(".qty_err")).text("Quantity doen't allow greater than " + qty)
            }
        })
    </script>
<script>
