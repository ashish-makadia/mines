...
class DumperMachineRegisterController extends Controller
{
    public function __construct()
    {
        $this->middleware(function ($request, $next) {
            $permission = Permission::where("name","dumpermachine_register")->first();
    ...
    public function index(Request $request){
        ...
        // dd($ac);
        if($request->ajax()) {
            $data =DumperMachineRegister::with('assetcategroy','assets')->orderBy('id', 'DESC')->select('*');
    ...
    public function update(Request $request){
        ...
        [
            ...
            $inputs = $request->except('_token','_method','editid');
