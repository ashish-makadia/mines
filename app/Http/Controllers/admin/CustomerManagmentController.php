...
class CustomerManagmentController extends Controller
{
     public function __construct()
    {
        $this->middleware(function ($request, $next) {
            $permission = Permission::where("name","customer_management")->first();
    ...
    public function store(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'customer_email'  => 'required|email|unique:customer_managments,customer_email',

            'customer_gst' => [
                'required',
                'regex:/^[0-9]{2}[A-Z]{5}[0-9]{4}[A-Z]{1}[1-9A-Z]{1}Z[0-9A-Z]{1}$/',
                'unique:customer_managments,customer_gst'
            ],
            'customer_pan' => [
                'required',
                'regex:/^[A-Z]{5}[0-9]{4}[A-Z]{1}$/',
                'unique:customer_managments,customer_pan'
            ],
            'state_id'  => 'required',
            'city_id'   =>  'required',
